...
parser.add_argument('--source', type=str, default='train_images/5.tif', help='path to images')
parser.add_argument('--output', type=str, default='output', help='path to outputs')
parser.add_argument('--targets', type=str, default='')
parser.add_argument('--weights', type=str, default='weights/best.pt')

parser.add_argument('--plot-flag', default=False, action='store_true')
parser.add_argument('--secondary-classifier', default=False, action='store_true')
parser.add_argument('--cfg', type=str, default='cfg/c3_a30symmetric.cfg', help='cfg file path')
parser.add_argument('--names', type=str, default='data/ldn.names', help='path to class label file')
parser.add_argument('--conf-thres', type=float, default=0.99, help='object confidence threshold')
parser.add_argument('--nms-thres', type=float, default=0.4, help='iou threshold for non-maximum suppression')
parser.add_argument('--iou-thres', type=float, default=0.5, help='iou threshold for detection scoring')
parser.add_argument('--batch-size', type=int, default=1, help='size of the batches')
parser.add_argument('--img-size', type=int, default=32 * 51, help='size of each image dimension')
parser.add_argument('--classes', type=int, default=3, help='number of classes for training')

opt = parser.parse_args()
print(opt)

def detect(opt):
    out_dir = str(Path(opt.output, datetime.now().strftime('%Y-%m-%d_%H-%M-%S')))  # Added to replace old way of just deleting previous results

    if opt.plot_flag:
        os.system('rm -rf ' + out_dir + '_img')
    ...
    # Set Dataloader
    classes = load_classes(opt.names)  # Extracts class labels from file
    dataloader = ImageFolder(opt.source, batch_size=opt.batch_size, img_size=opt.img_size, targets_path=opt.targets)

    imgs = []  # Stores image paths
    img_detections = []  # Stores detections for each image index
    prev_time = time.time()
    detections = None
    mat_priors = scipy.io.loadmat(opt.targets)
    ...
    for img_i, (path, detections) in enumerate(zip(imgs, img_detections)):
        ...
        if detections is not None:
            ...
            print("len = ", len(unique_classes))
            bbox_colors = random.sample(color_list, len(unique_classes))

            # write results to .txt file
            results_path = Path(out_dir) / Path(path).stem
            results_txt_path = results_path.with_suffix('.txt')
            if results_txt_path.is_file():
                os.remove(results_txt_path)

            results_img_path = results_path.with_suffix('.tif')
            with open(results_txt_path, 'a') as file:
                print("Writing", unique_classes, "classes to", results_txt_path)
                ...
                for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
                    ...
                    # if (xvc != 21) & (xvc != 72):
                    det_id+=1
                    file.write(('%g %g %g %g %g %g %g \n') % (x1, y1, x2, y2, xvc, cls_conf * conf, det_id))
    ...
    if opt.plot_flag:
        ...
        score.score(out_dir + '/', '/xview/xView_train.geojson', '.')  # TODO remove hardcoded path
...
class ConvNetb(nn.Module):
    def __init__(self, num_classes=60):
        ...
        self.fully_conv = nn.Conv2d(n * 16, 60, kernel_size=4, stride=1, padding=0, bias=True)  # TODO figure out if this is 60 classes or 60 anchor box dims
...
# dets_to_map( out_dir, opt.targets, out_dir+'\\shps' , opt.iou_thres, opt )