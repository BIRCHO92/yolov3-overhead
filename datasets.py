...
from utils.utils import xyxy2xywh, counts_to_weights


class ImageFolder():  # For Detection Only
    def __init__(self, source_path, batch_size=1, img_size=416, targets_path=''):
        
        if os.path.isdir(source_path):
            self.files = [str(img_path) for img_path in Path(source_path).rglob('*.tif')]
            print(self.files)
        elif os.path.isfile(source_path):
            self.mat = scipy.io.loadmat(source_path, squeeze_me = True)
            self.files = self.mat['img_paths']
...
class ListDataset():  # For training only
    def __init__(self, batch_size=1, img_size=608, targets_path=''):
        self.mat = scipy.io.loadmat(targets_path, squeeze_me = True)
        self.files = self.mat['img_paths']
        # self.mat['id'] = self.mat['id']
        self.nF = len(self.files)  # number of image files
        self.nB = math.ceil(self.nF / batch_size)  # number of batches
        self.batch_size = batch_size
        ...
        self.height = img_size
        # load targets
        self.num_classes = len(np.unique(self.mat['classes']))
        self.class_counts = np.unique(self.mat['classes'], return_counts=True)[1]
        self.class_weights = counts_to_weights(self.class_counts, range(self.num_classes))
    ...
    def __iter__(self):
        ...
        self.shuffled_vector = np.random.choice(self.mat['img_paths'].ravel(), self.nF,
    ...
    def __next__(self):
        ...
        for index, files_index in enumerate(range(ia, ib)):
            ...
            # Load labels
            chip = str( Path(img_path).stem)
            i = (self.mat['chips'] == chip)
