...
def insert_dir_in_paths(paths, new_dir):
    from pathlib import PurePath
    new_paths=[]
    for path in paths:
        path_list = list(PurePath(path).parts)
        path_list.insert(-3, new_dir)
        file_path1 = PurePath('').joinpath(*path_list)
        new_paths.append(str(file_path1))
    return new_paths
...
def counts_to_weights(counts, indices):  # weights of each class in the training set, normalized to mu = 1
